...
describe("WeeekApiClient", () => {
  ...
  describe("get", () => {
    ...
    it("returns parsed JSON on 200", async () => {
      fetchSpy.mockResolvedValueOnce(mockResponse({ projects: [{ id: 1 }] }));
      const client = new WeeekApiClient(token);
      const res = await client.get<{ projects: Array<{ id: number }> }>("/tm/projects");
    ...
    it("serializes query params", async () => {
      ...
      const client = new WeeekApiClient(token);
      await client.get("/tm/tasks", { limit: 20, offset: 0, projectId: "p1" });
    ...
    it("throws WeeekApiError with status 401 on 401 response", async () => {
      fetchSpy.mockResolvedValueOnce(mockResponse("unauthorized", { status: 401 }));
      const client = new WeeekApiClient(token);
    ...
    it("throws WeeekApiError with status 404 on 404 response", async () => {
      fetchSpy.mockResolvedValueOnce(mockResponse("not found", { status: 404 }));
      const client = new WeeekApiClient(token);
    ...
    it("throws WeeekApiError with status 500 on 500 response", async () => {
      fetchSpy.mockResolvedValueOnce(mockResponse("server boom", { status: 500 }));
      const client = new WeeekApiClient(token);
      await expect(client.get("/tm/projects")).rejects.toBeInstanceOf(WeeekApiError);
    });
    ...
    it("throws WeeekTimeoutError when fetch is aborted", async () => {
      ...
      await expect(client.get("/tm/projects")).rejects.toBeInstanceOf(WeeekTimeoutError);
    });
  });

  describe("post", () => {
    it("sends Content-Type JSON and stringified body", async () => {
      fetchSpy.mockResolvedValueOnce(mockResponse({ task: { id: 1 } }));
      const client = new WeeekApiClient(token);
      await client.post("/tm/tasks", { title: "hello", projectId: "p1" });

      const init = fetchSpy.mock.calls[0]![1] as RequestInit;
      const headers = init.headers as Record<string, string>;
      expect(headers["Content-Type"]).toBe("application/json");
      expect(init.body).toBe(JSON.stringify({ title: "hello", projectId: "p1" }));
